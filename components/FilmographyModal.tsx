...
const FilmographyModal: React.FC<FilmographyModalProps> = ({
  personId,
  personName,
  isOpen,
  onClose,
    ...
    if (isOpen && personId) {
      const fetchFilmography = async () => {
        ...
        try {
          ...
          if (import.meta.env.DEV) console.error('Erreur filmographie:', error);
        } finally {
          setLoading(false);
        }
      };
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 sm:p-6">
      ...
      <div className="relative z-10 bg-white w-full max-w-2xl rounded-[2.5rem] shadow-2xl flex flex-col max-h-[85vh] overflow-hidden animate-[scaleIn_0.3s_ease-out]">
        <div className="p-6 flex justify-between items-center border-b border-sand">
          <div>
            <h3 className="text-2xl font-black text-charcoal tracking-tight leading-none">
              Top 10
            </h3>
            <p className="text-[10px] font-black uppercase text-forest tracking-widest mt-1">
              Filmographie de {personName}
            </p>
          </div>
          <button
            onClick={onClose}
            className="p-2.5 bg-stone-100 rounded-full text-stone-500 hover:bg-stone-200 transition-all active:scale-90"
          >
