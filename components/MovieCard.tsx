...
const RatingBar = ({ label, value, hasPoster, isExpanded }: { label: string, value: number, hasPoster: boolean, isExpanded: boolean }) => {
  return (
    <div className="flex items-center gap-4 mb-4 group/bar">
      <span className={`text-[9px] font-black uppercase w-20 tracking-widest ${hasPoster ? 'text-white/40' : 'text-stone-400 dark:text-stone-500'}`}>
        {label}
      </span>
      <div className={`flex-1 h-1.5 rounded-full overflow-hidden ${hasPoster ? 'bg-white/10' : 'bg-stone-100 dark:bg-white/5'}`}>
        <div 
      ...
      <span className={`text-[10px] font-black w-6 text-right ${hasPoster ? 'text-white' : 'text-charcoal dark:text-white'}`}>
        {value}
      </span>
    </div>
  );
    ...
    <div className="relative overflow-hidden rounded-[2.5rem]" ref={cardRef}>
      ...
      >
        ...
        <div className="relative z-10 flex justify-between items-start mb-4">
          <span className={`text-[9px] font-black uppercase tracking-[0.2em] px-3 py-1.5 rounded-full border border-current/20 transition-colors duration-200 ${hasPoster ? 'bg-black/60' : 'bg-stone-50 dark:bg-[#161616]'}`}>
              {movie.genre}
          </span>
        ...
        <div className={`relative z-10 transition-[margin,transform] duration-300 ${isExpanded ? 'mb-6' : 'mt-auto'}`}>
          ...
          <div className={`flex items-center gap-3 uppercase tracking-widest text-[10px] font-black transition-colors duration-200 ${hasPoster ? 'text-white/60' : 'text-stone-400 dark:text-stone-600'}`}>
              <span>
        ...
        <div className={`relative z-10 grid transition-[grid-template-rows,opacity,transform] duration-300 ease-[cubic-bezier(0.16,1,0.3,1)] ${isExpanded ? 'grid-rows-[1fr] opacity-100' : 'grid-rows-[0fr] opacity-0 pointer-events-none'}`} style={{ willChange: 'opacity, transform' }}>
          <div className="overflow-hidden">
            {!isWatchlist && (
              <div className={`rounded-[2rem] p-6 mb-6 border transition-colors duration-200 ${hasPoster ? 'bg-[#0c0c0c] border-white/10' : 'bg-stone-50 dark:bg-[#0c0c0c] border-stone-100 dark:border-white/5'}`}>
                 <div className="flex justify-between items-center mb-6">
                    <span className={`text-[10px] font-black uppercase tracking-[0.2em] ${hasPoster ? 'text-white/30' : 'text-stone-300 dark:text-stone-700'}`}>Analyse Sensorielle</span>
            ...
            {movie.comment ? (
              <>
                  {movie.review && (
                      <div className="mb-3">
                          <p className={`text-[11px] leading-relaxed line-clamp-2 ${hasPoster ? 'text-white/40' : 'text-stone-400 dark:text-stone-600'}`}>
                              {movie.review}
                          </p>
                      </div>
                  )}
                  <div className={`mb-8 pl-5 border-l-2 transition-colors duration-200 ${hasPoster ? 'border-white/20' : 'border-stone-200 dark:border-stone-800'}`}>
                      <p className={`text-sm font-medium leading-relaxed italic ${hasPoster ? 'text-white/80' : 'text-stone-600 dark:text-stone-400'}`}>
                          "{movie.comment}"
                      </p>
                  </div>
              </>
