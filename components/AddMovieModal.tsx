        ...
        <div className="overflow-y-auto p-6 sm:p-8 space-y-8 no-scrollbar flex-1 pb-32">
          ...
          {!isEditMode && (
            <div className="space-y-6 relative">
              <div className="group">
                {/* Le type d'œuvre n'est plus un choix ici : il est celui de la
                    partie d'où l'on vient. Ce sélecteur était la dernière porte
                    par laquelle une série pouvait entrer dans la collection de
                    films — et un film dans celle des séries. La bascule en haut
                    de l'application décide, et elle décide seule. */}
                <p className="text-[10px] font-black uppercase tracking-[0.2em] text-stone-400 dark:text-stone-600 mb-3 ml-1 flex items-center gap-2">
                  {searchType === 'tv' ? <Tv size={12} /> : <Film size={12} />}
                  {t(searchType === 'tv' ? 'addMovie.series' : 'addMovie.movies')}
                </p>
