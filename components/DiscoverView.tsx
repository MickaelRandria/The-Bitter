...
const PROVIDERS = [
  { id: 'all', name: 'Tous', logo: null },
  {
    id: 8,
    name: 'Netflix',
    logo: 'https://upload.wikimedia.org/wikipedia/commons/thumb/7/75/Netflix_icon.svg/500px-Netflix_icon.svg.png',
  },
          ...
          <div className="absolute inset-y-0 right-2 flex items-center gap-2">
            ...
            {isSearchActive && !isAiSearching && (
              <button
                ...
                className="p-2 text-stone-300 dark:text-stone-700 hover:text-charcoal dark:hover:text-white"
              >
        ...
        {isSearchFocused && !isSearchActive && recentSearches.length > 0 && (
          <div className="flex flex-wrap items-center gap-2 px-1 animate-[fadeIn_0.2s_ease-out]">
            ...
            {recentSearches.map((s, i) => (
              <div
                key={i}
                className="flex items-center gap-1 bg-stone-100 dark:bg-[#202020] rounded-full pl-2.5 pr-1.5 py-1.5 border border-stone-200/50 dark:border-white/5"
              >
                <button
                  onMouseDown={(e) => {
                    e.preventDefault();
                  ...
                  className="text-[10px] font-bold text-stone-500 dark:text-stone-400 flex items-center gap-1.5"
                >
                  <History size={9} className="text-stone-300 dark:text-stone-600" />
                  {s}
                </button>
                <button
                  onMouseDown={(e) => {
                  ...
                  className="w-4 h-4 flex items-center justify-center text-stone-300 dark:text-stone-600 hover:text-charcoal dark:hover:text-white transition-colors"
                >
      ...
      {aiResult && (
        <div className="bg-stone-900 text-white rounded-[2.5rem] p-8 space-y-6 animate-[slideUp_0.4s_ease-out] border border-bitter-lime/20 relative overflow-hidden">
          ...
          <div className="flex items-center gap-3">
            ...
            <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-bitter-lime">
              Gemini Insight
            </h3>
          </div>
          <p
            ...
            <div className="grid grid-cols-3 gap-3">
              {[
                ...
                <button
                  ...
                  onClick={() => {
                    haptics.soft();
                    setTimePeriod(p.id as TimePeriod);
                  }}
                  className={`flex flex-col items-center justify-center gap-1.5 px-3 py-4 rounded-[1.8rem] transition-all border-2 ${timePeriod === p.id ? 'bg-forest border-forest text-white shadow-xl shadow-forest/10' : 'bg-white dark:bg-[#202020] text-stone-400 dark:text-stone-600 border-stone-100 dark:border-white/5 hover:border-stone-200 dark:hover:border-white/10'}`}
                >
                  {p.i}
                  <span className="text-[9px] font-black uppercase tracking-widest">{p.l}</span>
                </button>
              ))}
            </div>
          </div>
            ...
            <div className="flex p-1.5 bg-stone-100 dark:bg-[#161616] rounded-2xl border border-stone-200/50 dark:border-white/5 w-full shadow-inner transition-colors">
              {(['popularity', 'date', 'alpha'] as SortOption[]).map((opt) => (
                <button
                  ...
                  className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${sortBy === opt ? 'bg-white dark:bg-[#202020] text-forest dark:text-bitter-lime shadow-sm dark:shadow-black/30' : 'text-stone-400 dark:text-stone-600'}`}
                >
                  {opt === 'popularity' ? (
                    <Flame size={14} />
                  ) : opt === 'date' ? (
                ...
                >
                  <div className="relative aspect-[2/3] rounded-[2.5rem] overflow-hidden shadow-sm dark:shadow-black/20 group-hover:shadow-xl dark:group-hover:shadow-black/40 group-hover:-translate-y-1 transition-all duration-500 bg-stone-100 dark:bg-[#161616]">
                    ...
                    {isInWatchlist && !isWatched && (
                      <div className="absolute bottom-3 left-3 z-20 bg-orange-400/90 text-white px-2 py-1 rounded-full flex items-center gap-1 text-[9px] font-black backdrop-blur-sm shadow">
                        <Bookmark size={9} fill="currentColor" /> Watchlist
                      </div>
                    )}
                    {/* Action buttons */}
                    {!isInCollection && (
                      <div className="absolute bottom-3 right-3 opacity-0 group-hover:opacity-100 transition-all duration-300 translate-y-2 group-hover:translate-y-0 flex gap-2 z-20">
                        {onQuickWatchlist && (
                          <button
                            onClick={(e) => {
                              ...
                              onQuickWatchlist(item.id, mediaType).finally(() =>
                                setQuickAddingId(null)
                              );
                            }}
                            disabled={quickAddingId === item.id}
                            className="w-10 h-10 bg-white/90 dark:bg-[#1a1a1a]/90 text-charcoal dark:text-white rounded-full flex items-center justify-center shadow-lg backdrop-blur-sm active:scale-95 transition-transform"
                          >
                            {quickAddingId === item.id ? (
                        ...
                        <button
                          ...
                          className="w-10 h-10 bg-forest dark:bg-lime-500 text-white dark:text-black rounded-full flex items-center justify-center shadow-lg active:scale-95 transition-transform"
                        >
                  ...
                  <div className="px-1">
                    <h4 className="text-sm font-black text-charcoal dark:text-white leading-tight line-clamp-2 mb-1 group-hover:text-forest dark:group-hover:text-lime-500 transition-colors">
                      {item.title || item.name}
                    </h4>
                    <p className="text-xs text-stone-500 dark:text-stone-600 font-semibold">
                      {(item.release_date || item.first_air_date)?.split('-')[0]}
                      {item.vote_count && item.vote_count > 0 ? (
                        <span className="ml-2 text-[9px] font-bold text-stone-400">
                          💬{' '}
