...
const AuthScreen: React.FC<AuthScreenProps> = ({ onContinueAsGuest }) => {
  ...
  const handleForgotPassword = async (e: React.FormEvent) => {
    ...
    try {
      // Fix: Use correct redirectTo for SPA (root with query param)
      // @ts-ignore - Property does not exist on type SupabaseAuthClient
  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    try {
      if (authMode === 'signup') {
        ...
        // Utilisation directe de l'API v2 sans cast 'as any'
        // @ts-ignore - Property does not exist on type SupabaseAuthClient
    ...
    <div className="min-h-screen bg-cream flex flex-col relative overflow-hidden font-sans selection:bg-forest selection:text-white">
      ...
      {resetEmailSent ? (
        <div className="flex-1 flex flex-col items-center justify-center px-6 relative z-10 w-full max-w-md mx-auto text-center">
            ...
            <button
            onClick={() => { setResetEmailSent(false); setAuthMode('login'); setError(null); haptics.soft(); }}
                ...
                <form onSubmit={handleAuth} className="space-y-5">
                    ...
                    >
                        {loading ? (
                            ...
                            <>
                                {authMode === 'signup' ? "Créer le compte" : authMode === 'forgot' ? "Envoyer le lien" : "Entrer"}
