...
const MovieDeck: React.FC<MovieDeckProps> = ({
  onRate,
  onClose,
  advanceTrigger = 0,
  favoriteGenres = [],
  ...
  useEffect(() => {
    const fetchPersonalized = async () => {
      ...
      try {
        ...
        if (import.meta.env.DEV) console.error('Deck error', e);
      } finally {
        setLoading(false);
      }
    };
    ...
    return (
      <div className="flex flex-col items-center justify-center py-32 gap-6">
        ...
        <p className="text-[10px] font-black uppercase tracking-[0.3em] text-stone-400 animate-pulse">
          Filtrage de votre zone de confort...
        </p>
      </div>
    );
  ...
  return (
    <div className="relative w-full max-w-sm mx-auto h-[75vh] flex flex-col items-center justify-center px-4">
      ...
      <div
        key={movie.id}
        className="relative w-full h-full bg-white rounded-[2.5rem] shadow-2xl overflow-hidden animate-[scaleIn_0.4s_cubic-bezier(0.16,1,0.3,1)] border border-stone-100"
      >
        <img
      ...
      <div className="mt-8 flex items-center gap-1.5">
        {movies.map((_, i) => (
          <div
            key={i}
            className={`h-1 rounded-full transition-all duration-500 ${i === currentIndex ? 'w-8 bg-charcoal' : 'w-2 bg-stone-200'}`}
          />
        ))}
      </div>
    </div>
