...
const movieToRow = (movie: Movie, profileId: string) => ({
  ...
  date_watched: movie.dateWatched || null,
...
const rowToMovie = (row: any): Movie => ({
  ...
  dateAdded: typeof row.date_added === 'number' ? row.date_added : new Date(row.date_added).getTime(),
  dateWatched: row.date_watched ? (typeof row.date_watched === 'number' ? row.date_watched : new Date(row.date_watched).getTime()) : undefined,
  ratings: {
